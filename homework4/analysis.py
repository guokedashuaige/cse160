...
def update_assignment(list_of_points, labels, centroids_dict):
    ...
    for i, pt in enumerate(list_of_points):
        cpt = get_closest_centroid(pt, centroids_dict)
        if cpt in dd:
            dd[cpt].append(labels[i])
        else:
            dd[cpt] = [labels[i]]
    return dd
...
def majority_count(labels):
    ...
    return max([labels.count(i) for i in set(labels)])
...
def accuracy(list_of_points, labels, centroids_dict):
    ...
    dd = update_assignment(list_of_points, labels, centroids_dict)
    for i in dd.values(): print(i)
    n = sum([majority_count(i) for i in dd.values()])
    return n / len(labels)
...
# accuracy: 0.582
# Digits with less overlapping with others are easier to be distinguished,
# like 7, 2, 1, 0. Otherwise they are harder to be distinguished from each
# other, like (3, 5, 8), (4, 9).
