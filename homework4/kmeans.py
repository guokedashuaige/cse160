...
def get_closest_centroid(point, centroids_dict):
    ...
    dmin = math.inf
    centroid1 = ''
    for name, pos in centroids_dict.items():
        d = euclidean_distance(point, pos)
        if d < dmin:
            dmin = d
            centroid1 = name
    return centroid1
...
def mean_of_points(list_of_points):
    ...
    return [sum(x)/len(list_of_points) for x in zip(*list_of_points)]
...
def update_centroids(assignment_dict):
    ...
    dd = {}
    for name, pt_ in assignment_dict.items():
        dd[name] = mean_of_points(pt_)
    return dd
